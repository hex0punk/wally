...
- `4` (very-strict): Wally will match node names to call site names (e.g., if function `foo` can only be called by the name `foo`). This will lead to the most accurate results, but in some cases (due to the large number of spurious nodes generated by the Go callgraph algorithms) this could take a while to finish. 
